...
public class UI extends JFrame implements Runnable{
	...
	public UI() {
		...
		numberSpinner.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if((int)numberSpinner.getValue() > 12) {
					numberSpinner.setValue(12);
				}
			}
	...
	private ArrayList<line> createLines(int[] path) {
		...
		for(int index = 0; index < path.length - 1; index++) {
			...
			//point p1 = new point(mp.points.get(path[index]).x, mp.points.get(path[index]).y);
			//point p2 = new point(mp.points.get(path[index + 1]).x, mp.points.get(path[index + 1]).y);
	...
	private void updateDistance() {
		...
		if(totalDist == Integer.MAX_VALUE) 		DistanceLabel.setText(DistanceTxt + 0);
		else DistanceLabel.setText(DistanceTxt + totalDist);
	...
	public void paintComponent(Graphics g) {
		...
		if(points.size() > 0) {
			for(int index = 0; index < points.size(); index++) {
				...
				g.drawString("(" + points.get(index).x + ", " + points.get(index).y + ")", points.get(index).x + 5, points.get(index).y + 5);
			}
		...
		if(shortestLines.size() > 0) {
			for(int index = 0; index < shortestLines.size(); index++) {
				g.drawLine(shortestLines.get(index).x1, shortestLines.get(index).y1, shortestLines.get(index).x2, shortestLines.get(index).y2);
			}
		}
		g.setColor(Color.white);
		if(currentLines.size() > 0) {
			for(int index = 0; index < currentLines.size(); index++) {
				g.drawLine(currentLines.get(index).x1, currentLines.get(index).y1, currentLines.get(index).x2, currentLines.get(index).y2);
			}
		}
